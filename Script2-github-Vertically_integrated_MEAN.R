...
#   Vertically integrated data MEAN
...
# With this scripts we generate vertically integrated matrix per year and month  (weighted MEAN).
...
# Calculate thw width of each dpeth layer
espesor_depth <-diff(depth)
# Identify the depth layer for which we want to do a subset of your data (this value can be changed)
ind<-which.min(abs(depth-z)) #in this case z=400 as defined at the beginning pf the script
# Create a subset of depth from layer 1 up to the selected depth, in this case 400m
depth2<-depth[1:ind]
...
image.plot(temp_combined[,,1,12]) # plot random slices of the matrix to check it
...
# Create an empty matrix to store the outputs of the loop
temp_matrix<-array(NA,dim=c(length(lon),length(lat),total_time))
str(temp_matrix)

#loop 
# weighted MEAN vertical integration
for (i in 1:total_time){  
  print(paste("Month",i))
  for (j in 1:length(lon)){
    for (k in 1:length(lat)){
      temp_ <- temp_combined[j,k,,i] #access the temp value of lon (j), lat(k) and month(i)
      indices <- which(!is.na(temp_), arr.ind = TRUE)
      temp_m<-sum(temp_*espesor_depth2, na.rm=T)/sum(espesor_depth2[indices])#multiply each cell (pixel) by the width of the layer, and sum all of them together then divide by the sum of all the depths total depth
...
#SAVE surface matrix. This matrix can be used for the Cumulative Impact analysis based on monthly anomalies
#saveRDS(temp_matrix, file = "./Outputs/Pre-process/Temperature/v_int_MEAN_temp_matrix_Black_Sea_1993_2021.RData")
...
image.plot(temp_mean, main="Mean v_int MEAN Temperature 1992-2022")

#SAVE vertically integrated matrix. This matrix can be used for the Cumulative Impact analysis based on mean annual values
#saveRDS(v_int_temp_yearly_matrix, file = "./Outputs/Pre-process/Temperature/v_int_MEAN_temp_matrix_YEARLY_Black_Sea_1993_2021.RData")
