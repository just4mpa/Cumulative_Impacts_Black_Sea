...
# Here, this script is applied to Chlorophylla data but it can be used for all the variables we want to include in the cumulative impacts analysis.
...
# If you don't need a subset, you stilll need to define the min and max of your entire dataset
# These values must be EXACTLY the SAME as in the nc. (otherwise it won't work)

# Select one or the other according to the area you need
...
ind<-which.min(abs(depth-z)) #in this case z=150 as defined at the beginning pf the script
...
f.int.v2 <- function(xx) {
  if (all(is.na(xx))) {
    ...
    chla.int <- sum(vals.int) # each Chla value is attributed to the 1 m layer below  #SUM interpolated values
    return(chla.int)
  }
}

# The function uses PCHIP (Piecewise Cubic Hermite Interpolating Polynomial) to interpolate the Chla values (vvv) at the specified depths (ddd) over the new depth points (pp).
# The result, vals.int, is a vector of interpolated Chl-a values corresponding to each depth in pp.
...
for (i in 1:total_time){  
  ...
  for (j in 1:length(lon)){
    for (k in 1:length(lat)){
      ...
      chl_m <- f.int.v2(chl_) #apply the function
...
#saveRDS(chl_matrix, file = "./Outputs/Pre-process/Chl/v_int_SUM_interpolation_chl_Black_sea_1992_2022.RData")
...
str(v_int_chl_yearly_matrix)# Now the new matrix must have the yearly temporal dimension
...
image.plot(chl_mean, main="Mean v_int SUM Chl (1m interpolation)")

#SAVE matrix. his matrix can be used for the Cumulative Impact analysis based on mean annual values
#saveRDS(v_int_chl_yearly_matrix, file = "./Outputs/Pre-process/Chl/v_int_SUM_interpolation_chl_matrix_YEARLY_Black_Sea_1992_2022.RData")
